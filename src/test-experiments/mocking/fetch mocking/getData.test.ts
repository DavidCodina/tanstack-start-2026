...
describe('getData()', () => {
  ...
  test('should also work', async () => {
    ...
    try {
      ...
      expect(data).toBeNull() // eslint-disable-line
      expect(success).toBe(false) // eslint-disable-line
      expect(message).toBe('Request not successful. (1)') // eslint-disable-line

      expect(global.fetch).toHaveBeenCalledTimes(2) // eslint-disable-line
      expect(global.fetch).toHaveBeenCalledWith(URL) // eslint-disable-line
    }
  })
})
